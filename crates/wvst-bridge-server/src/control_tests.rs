...
async fn creates_lists_and_destroys_instance() {
    ...
    .to_string();
    let status = handle_control_text(
        &status_request,
        ControlContext {
            config: &config,
            host_worker: &host_worker,
    ...
    .await;
    let status_value: Value = serde_json::from_str(&status.text).expect("valid status json");
...
while IFS= read -r line; do
  ...
  case "$line" in
    *worker.hello*) printf '{"jsonrpc":"2.0","id":%s,"result":{"workerName":"test-worker","ipcVersion":1}}\n' "$id" ;;

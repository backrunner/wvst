...
- Bridge 控制面已提供 `instance.metadata.refresh` 批量刷新入口，可通过 framed control batch 在一次 worker 往返中获取参数列表、unit metadata、可选 state 和默认 worker metrics；JSON-line 测试 worker 会自动 fallback 为顺序请求。

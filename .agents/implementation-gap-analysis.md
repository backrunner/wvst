...
- late/drop/jitter/backpressure 首版 Bridge 诊断指标已完成，`wvst-testkit` 已提供可复用 latency snapshot collector、30 分钟 stability runner 和可序列化 budget evaluator，并可把 WebAudio underflow/overflow/pending quantum 与 Bridge 序号/延迟指标放进同一预算报告；仍缺少真实 Web SDK 端自动采集接入、真实端到端 WebAudio 往返延迟测量和共享内存数据面。

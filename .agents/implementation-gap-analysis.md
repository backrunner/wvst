...
- late/drop/jitter/backpressure 首版 Bridge 诊断指标已完成，`wvst-testkit` 已提供可复用 latency snapshot collector、30 分钟 stability runner 和可序列化 budget evaluator；仍缺少 WebAudio 端 underflow/overflow 与 Bridge 序号指标的统一策略、真实端到端 WebAudio 往返延迟测量和共享内存数据面。

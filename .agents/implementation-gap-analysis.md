...
- `wvst-testkit` 已新增纯 Rust latency harness，可记录 sequence gap、duplicate/out-of-order、drop、late/silence/process-error 计数，并输出 route latency、round-trip frames 与 round-trip microseconds 的 p50/p95/p99 快照，后续 Bridge/Web/真实插件稳定性测试可复用。
...
- late/drop/jitter/backpressure 首版 Bridge 诊断指标已完成，`wvst-testkit` 已提供可复用 latency snapshot collector；仍缺少 WebAudio 端 underflow/overflow 与 Bridge 序号指标的统一策略、真实端到端 WebAudio 往返延迟测量、30 分钟稳定性 harness runner 和共享内存数据面。

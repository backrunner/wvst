...
- 真实第三方插件兼容验证仍不足；当前 `setProcessing`、`process`、process context、latency/tail 和 output audio health 主要由 fake ABI fixture、worker passthrough、Bridge runtime-info 传播测试、可对真实 bundle 执行的多 block `runtime-probe` smoke 入口和 `wvst-testkit` runtime matrix runner 覆盖，仍需要在 CI/本地纳入固定第三方插件清单与长时稳定性记录。

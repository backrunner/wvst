...
- Workspace 已新增 Rust-first `wvst-packager` crate，`wvst-packager macos` 可生成包含 bridge/host-worker 二进制、launcher、LaunchAgent plist、env 示例、安装脚本、卸载脚本和 README 的 macOS release bundle；`wvst-packager linux` 可生成包含 bridge/host-worker 二进制、launcher、systemd user service、env 示例、安装脚本、卸载脚本和 README 的 Linux release bundle；`wvst-packager windows` 可生成包含 bridge/host-worker `.exe`、PowerShell launcher、env 示例、Scheduled Task 安装/卸载脚本和 README 的 Windows release bundle；安装脚本会生成本机 token、安装用户级启动项并保留配置以便排障。
...
- macOS 安装、启动、授权和日志已有基础诊断命令支持，`wvst-packager macos` 已提供首版发布 bundle 生成、安装和卸载脚本；Linux 已提供 systemd user service 发布 bundle 生成、安装和卸载脚本；Windows 已提供 Scheduled Task 发布 bundle 生成、安装和卸载脚本；仍缺少 macOS 签名/公证、三平台真实安装验证和更完整的日志采集/轮转命令。

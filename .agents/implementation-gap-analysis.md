...
- `wvst-host-worker runtime-probe <plugin.vst3> <class-id>` 已提供隔离进程内真实 VST3 runtime smoke 入口，可按指定 sample rate、block size、输入/输出通道、frames、blocks、MIDI note 和参数自动化输入创建 component/controller、完成 setup/activate/start/process/stop/terminate，并输出参数数量、controller 参数 flag 计数、component/controller state 读取摘要、unit/program metadata 与 program/unit data 支持摘要、audio bus、latency/tail、process context requirements、process timing、output event/parameter diagnostics、output energy/RMS、silent/non-zero block、first non-zero block/frame/absolute-frame、peak block、non-finite、clipped sample 计数，以及 note-on/note-off 到首个非零输出的 frame/microsecond timing JSON；process 失败时会尽量执行 stop/terminate 清理，便于第三方 effect/instrument 兼容验证自动化。
...
- `wvst-testkit` 已新增纯 Rust latency harness、stability runner、stability budget evaluator、scripted latency snapshot builder 和 runtime probe matrix runner，可记录 sequence gap、duplicate/out-of-order、drop、timeout、late/silence/process-error 计数，并输出 route latency、round-trip frames 与 round-trip microseconds 的 p50/p95/p99 快照；runner 默认按 30 分钟稳定性窗口推进 block/sequence/frame timeline，`wvst-latency-snapshot` CLI 可把 schema version 1 逐块观测 JSON 规范化为预算可消费的 `LatencySnapshot`，budget evaluator 可把最小观测数、drop/timeout/late/silence/process-error、序号异常、route/round-trip p95/p99，以及 WebAudio loopback input/output frames、underflow/overflow、pending input/output quantum 阈值转换为可序列化 pass/fail 报告，`wvst-stability-budget` CLI 可读取 latency snapshot、budget 和可选 WebAudio metrics JSON 并用退出码表达是否达标，便于真实数据面压测进入 CI；runtime matrix runner 可用固定 case 清单调用隔离 worker 的 `runtime-probe` 批量记录 effect/instrument、MIDI note 和参数自动化 smoke 结果；runtime matrix 已支持 schema version 1 JSON manifest 解析/校验，`.agents/runtime-probe-matrix.example.json` 提供 effect automation 与 zero-input instrument note 示例，manifest 可通过 `fixtureRoot`、`${ENV_VAR}` 和 `~/` 路径展开适配本机/CI 的真实 VST3 fixture 目录，并新增 `wvst-runtime-matrix` CLI 输出 schema version 1 JSON report、通过/失败退出码、顶层 `audioHealth` / `noteTiming` 汇总、compatibility/classification 诊断分类汇总和 case-level expectations；fixture 可声明预期 raw probe status、非静音/静音、note response、note-to-audio frame/microsecond latency、non-finite、clipped、RMS/peak 阈值以及预期 compatibility/classification category，未满足时会以 `expectation-failed` 标记矩阵失败，预期的负向 VST3 兼容失败也能保留 raw `probeStatus` 后作为 case 通过；`runtime-probe` 失败也会输出 schema version 1 JSON failure report，matrix result 会保留 `probeReport` 便于真实插件兼容问题聚合，后续 Bridge/Web/真实插件稳定性测试可复用。
...
- 真实第三方插件兼容验证仍不足；当前 controller 参数枚举、state 读取、unit/program metadata、program/unit data 支持、`setProcessing`、`process`、process context、latency/tail、output audio health 和 zero-input instrument note timing 主要由 fake ABI fixture、worker passthrough、Bridge runtime-info 传播测试、可对真实 bundle 执行的多 block `runtime-probe` smoke 入口和 `wvst-testkit` runtime matrix runner 覆盖，matrix report 已能汇总全静音、non-zero、non-finite、clipped、peak/RMS、note response 缺失和最慢 note-to-audio timing 异常线索；仍需要在 CI/本地纳入固定第三方插件清单与长时稳定性记录。
...
- late/drop/jitter/backpressure 首版 Bridge 诊断指标已完成，`wvst-testkit` 已提供可复用 latency harness、scripted latency snapshot CLI、30 分钟 stability runner 和可序列化 budget evaluator，并可把 WebAudio underflow/overflow/pending quantum 与 Bridge 序号/延迟指标放进同一预算报告；仍缺少真实 Web SDK 端自动采集接入、真实端到端 WebAudio 往返延迟测量和共享内存数据面。
...
- 音源 VST 的 zero-input audio buffer/session/passthrough worker plumbing 已有首版；note on/off 已能随 block 进入真实 VST3 `process()`，`runtime-probe` 已能记录 note-on/note-off 绝对 frame、首个非零输出 frame 和 note-to-audio latency，并可通过 matrix expectations 约束 `requireNoteResponse`、`maxNoteToAudioFrames` 和 `maxNoteToAudioMicros`；仍缺少真实第三方 instrument 兼容测试、Web worker 侧 late-event 策略和 Web MIDI adapter/虚拟键盘示例。
...
- macOS 安装、启动、授权和日志已有基础诊断命令支持，`wvst-packager macos` 已提供首版发布 bundle 生成、安装、卸载和诊断脚本；Linux 已提供 systemd user service 发布 bundle 生成、安装、卸载和诊断脚本；Windows 已提供 Scheduled Task 发布 bundle 生成、安装、卸载和诊断脚本；仍缺少 macOS 签名/公证、三平台真实安装验证和更完整的日志采集/轮转命令。
- Windows 真实运行验证和 Linux cgroup 资源限制真实部署验证。

## 建议下一阶段

1. 用带 case-level expectations 的 `wvst-testkit` runtime matrix JSON manifest 与 `wvst-runtime-matrix` CLI 接入真实 macOS VST3 effect/instrument fixture，验证 2-in/2-out process、parameter automation、MIDI mapping、zero-input instrument timing、输出音频健康和已知兼容性失败分类。
